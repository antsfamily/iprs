...
y1 = iprs.corr1(x, h, shape='same')
y2 = iprs.fftcorr1(x, h, axis=0, Nfft=Nfft, shape='same', ftshift=ftshift)
print(np.sum(np.abs(y1 - y2)), np.sum(np.angle(y1) - np.angle(y2)))

# y1 = np.convolve(x, h, mode='valid')
y1 = iprs.corr1(x, h, shape='valid')
y2 = iprs.fftcorr1(x, h, axis=0, Nfft=Nfft, shape='valid', ftshift=ftshift)
print(np.sum(np.abs(y1 - y2)), np.sum(np.angle(y1) - np.angle(y2)))

# y1 = np.convolve(x, h, mode='full')
y1 = iprs.corr1(x, h, shape='full')
y2 = iprs.fftcorr1(x, h, axis=0, Nfft=Nfft, shape='full', ftshift=ftshift)
print(np.sum(np.abs(y1 - y2)), np.sum(np.angle(y1) - np.angle(y2)))
